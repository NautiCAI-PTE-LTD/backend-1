...
MODEL_PATH = os.path.join(os.path.dirname(__file__), "biofouling_best.pt")

RESNET_MODEL_PATH = os.path.join(os.path.dirname(__file__), "resnet50_species_full_model.pt")

# ResNet-50 species classes (from model_info_resnet50.json)
SPECIES_CLASSES = ["algae", "barnacles", "mussels"]

try:
    import torchvision.transforms as transforms
    from torchvision.models import resnet50
    # The transform pipeline used for ResNet inference
    ...
    print(f"Loading ResNet species model from {RESNET_MODEL_PATH} on {device}...")
    # Load the full model
    resnet_model = torch.load(RESNET_MODEL_PATH, map_location=device)
    resnet_model.eval()
    print("ResNet species model loaded ✓")
...
async def detect(file: UploadFile = File(...)):
    ...
    if result.boxes is not None and len(result.boxes) > 0:
        ...
        for i in range(len(cls_ids)):
            ...
            # If it's biofouling/marine growth, run species classification
            # We crop the bounding box and pass it through ResNet
            is_growth = "growth" in class_name.lower() or "biofouling" in class_name.lower() or "fouling" in class_name.lower()
            if is_growth and resnet_model is not None:
                try:
                    # Crop image taking care of boundaries
                    ...
                    if cx2 > cx1 and cy2 > cy1:
                        ...
                        input_tensor = resnet_transform(crop_rgb).unsqueeze(0).to(device)
                        ...
                        species_name = SPECIES_CLASSES[top_class_idx.item()]
                        species_conf = top_prob.item()
                        
                        detection_obj["species"] = species_name
                        detection_obj["species_confidence"] = float(round(species_conf, 4))
                except Exception as e:
                    print(f"Species classification failed for bounding box: {e}")
